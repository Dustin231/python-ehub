...
#    model.CRFnet = Param(model.LinkID, domain=NonNegativeReals, initialize=data.NetCRF()) # network captial recovery factor
    ...
    model.impCost = Param(model.EImpSet, initialize=data.impCost.to_dict()) # EC import cost
    model.expPrice = Param(model.EExpSet, initialize=data.expPrice.to_dict())  # EC export price
